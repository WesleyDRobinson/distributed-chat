...
const buildContent = {
  image: async function ({ src, localSrc, message, ...rest }) {
    return wire()`<img src=${src} alt=${message}>`
  },
  text: async function ({ message, ...rest }) {
    return message
  },
}
...
const logError = function loadAndReturn(msg, e) {
  console.error(msg, e)
  return { msg, err }
}

const prepareFilesForUpload = async function prepareFilesForUpload(form) {
